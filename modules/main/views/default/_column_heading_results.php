...
<?php if($parent_column_heading_classes): ?>
    <?php foreach($parent_column_heading_classes as $parent_column_heading_class): ?>
        <h2><?= Yii::t('app', 'TABLE_ANNOTATION_PAGE_PARENT_CLASSES_FOR_ENTITY'); ?>
            <a href="<?= $parent_column_heading_class['result']['rows'][0]['callret-0']; ?>">
                <?= $parent_column_heading_class['result']['rows'][0]['callret-0']; ?>
            </a>
        </h2>
        <table class="table table-striped table-bordered">
            <tr>
            ...
            <?php foreach($parent_column_heading_class['result']['rows'] as $row): ?>
                <tr>
                    <?php foreach($parent_column_heading_class['result']['variables'] as $variable): ?>
                        <td><?= $row[$variable]; ?></td>
                    <?php endforeach; ?>
                </tr>
            <?php endforeach; ?>
